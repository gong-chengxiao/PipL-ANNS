...
class QGBuilder {
   private:
    ...
    void fill_remaining_neighbors(const CandidateList&, CandidateList&, std::vector<bool>&, const float*, PID, bool);
    float compute_angle(const float*, const float*, const float*);
    float compute_dynamic_theta(size_t, size_t);
...
inline void QGBuilder::heuristic_prune(
    ...
    const float* query_vec = qg_.get_vector(cur_id);
    ...
    while (pruned_results.size() < degree_bound_ && start < poolsize) {
        ...
        const float* candidate_vec = qg_.get_vector(candidate_id);
        float dist_qc = pool[start].distance;

        for (const auto& neighbor : pruned_results) {
            const float* neighbor_vec = qg_.get_vector(neighbor.id);
            float dist_nc = dist_func_(neighbor_vec, candidate_vec, dim_);
            
            // Distance constraint: dist(q, c) < dist(n, c)
            if (dist_qc >= dist_nc) {
                should_add = false;
                break;
            }
            
            // Angular constraint: angle(c, q, n) > theta
            float angle = compute_angle(candidate_vec, query_vec, neighbor_vec);
            float theta = compute_dynamic_theta(pruned_results.size(), degree_bound_);
        ...
        if (should_add) {
            ...
            if (refine && pruned_neighbors_[cur_id].size() < max_pruned_size_) {
                pruned_neighbors_[cur_id].emplace_back(pool[start]);
            }
            pruned[start] = true;
        }
...
inline float QGBuilder::compute_angle(const float* vec_a, const float* center, const float* vec_b) {
    std::vector<float> ca(dim_), cb(dim_);
    for (size_t i = 0; i < dim_; ++i) {
        ca[i] = vec_a[i] - center[i];
        cb[i] = vec_b[i] - center[i];
    }
    
    float dot_product = 0.0f;
    float norm_ca = 0.0f, norm_cb = 0.0f;
    
    for (size_t i = 0; i < dim_; ++i) {
        dot_product += ca[i] * cb[i];
        norm_ca += ca[i] * ca[i];
        norm_cb += cb[i] * cb[i];
    }
    
    norm_ca = std::sqrt(norm_ca);
    norm_cb = std::sqrt(norm_cb);
    
    if (norm_ca < 1e-8 || norm_cb < 1e-8) {
        return 0.0f;
    }
    
    float cos_angle = dot_product / (norm_ca * norm_cb);
    cos_angle = std::max(-1.0f, std::min(1.0f, cos_angle));  // Clamp to [-1, 1]
    
    return std::acos(cos_angle);
}

inline float QGBuilder::compute_dynamic_theta(size_t current_neighbors, size_t target_degree) {
    if (current_neighbors == 0) {
        return 0.0f;
    }
    
    float base_theta = M_PI / 6.0f;  // 30 degrees as base
    float remaining_ratio = static_cast<float>(target_degree - current_neighbors) / target_degree;
    float dynamic_theta = base_theta * (1.0f + (1.0f - remaining_ratio));

    return std::max<float>(M_PI / 12.0f, std::min<float>(M_PI / 3.0f, dynamic_theta));
}
