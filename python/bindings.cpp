...
struct Index {
    ...
    void set_num_pool_blocks(size_t num_pool_blocks) const { index->set_num_pool_blocks(num_pool_blocks); }

    void set_insert_limit(size_t limit) const { index->set_insert_limit(limit); }

    void set_num_scanners(size_t num_scanners) const { index->set_num_scanners(num_scanners); }

    void set_filter_alpha(float filter_alpha) const { index->set_filter_alpha(filter_alpha); }
...
PYBIND11_MODULE(symphonyqg, m) {
    ...
    py::class_<Index>(m, "Index")
        ...
        .def("set_num_pool_blocks", &Index::set_num_pool_blocks, py::arg("num_pool_blocks"))
        .def("set_insert_limit", &Index::set_insert_limit, py::arg("limit"))
        .def("set_num_scanners", &Index::set_num_scanners, py::arg("num_scanners"))
        .def("set_filter_alpha", &Index::set_filter_alpha, py::arg("filter_alpha"))
        .def(
            "build_index",
            &Index::build_index,
            py::arg("data"),
